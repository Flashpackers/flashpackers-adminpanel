...
function AuthFormLayout1({ sendtoparent }) {
    ...
    const loginurl = 'https://1d63-119-160-199-91.ngrok-free.app/api/admin/loginAuth'
    const verifyuserurl = 'https://1d63-119-160-199-91.ngrok-free.app/api/admin/verifyUser'
    ...
    const Handlesubmit = async (event) => {
        ...
        try {
            ...
            const validtoken = await axios.post(verifyuserurl, credential);
            sendtoparent(validtoken.data.valid);
            setData(validtoken.data);
        }
        catch (error) {
    ...
    useEffect(() => {
        ...
        if (token) {
            const verifyToken = async () => {
                try {
                    const isvalid = await axios.post(verifyuserurl, credential);
                    let verified = isvalid.data.valid;
                    if (verified) {
                        sendtoparent(verified);
                        setData(isvalid.data);
                    }
    ...
    return (
        <div className="flex h-lvh sm:justify-center justify-center bg-green-100 lg:justify-start items-center">
            <div className="mx-12 mb-[65px] flex items-center justify-center lg:max-w-lg">
                <div className="flex h-full flex-col">
                    ...
                    <form className="flex flex-col relative pl-6" onSubmit={Handlesubmit}>
                        ...
                        <button
                            type="submit"
                            className="relative top-10 hover:cursor-pointer sm:w-96 lg:w-[382px] lg:h-12 w-64 shadow-lg shadow-gray-400 rounded-lg bg-green-500 text-lg flex items-center justify-center text-black hover:bg-green-400"
                        >
                            Log In
                        </button>
                    </form>
